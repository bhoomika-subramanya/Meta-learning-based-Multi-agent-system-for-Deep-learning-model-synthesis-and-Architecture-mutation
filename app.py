...
class NeuralArchitecture(nn.Module):
    def __init__(
        ...
        current_dim = input_dim[0] * input_dim[1]
        ...
        for i, block_config in enumerate(blocks):
            ...
            if block_type == "conv":
                ...
                current_dim = input_dim[0] * input_dim[1] * current_channels
        ...
        if blocks and blocks[-1]["type"] == "conv":
            ...
            self.output_layer = nn.Linear(
                current_channels * input_dim[0] * input_dim[1],
                output_dim
            )
        else:
            self.flatten = None
    ...
    def forward(self, x):
        if self.input_is_conv:
            ...
            x = x.view(-1, self.input_dim[0] * self.input_dim[1])
    ...
    def mutate_architecture(self, architecture: NeuralArchitecture) -> NeuralArchitecture:
        ...
        if random.random() < self.mutation_rate:
            ...
            if mutation_type == "add" and len(blocks) < 5:
                ...
                blocks[block_idx]["out_features"] = random.choice(
                    [16, 32, 64, 128, 256]
                )
...
class MetaLearningController:
    ...
    def run(self):
        ...
        population = [
            self.generator.generate_architecture(input_channels, input_dim, output_dim)
            for _ in range(5)
        ]
